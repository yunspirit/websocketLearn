package learn_websocket.service;


import learn_websocket.controller.v5.StockService;
import learn_websocket.controller.v6.User;
...
public class WebSocketService {
    ...
    public void sendStockInfo() {
        ...
        if (null != stockInfoMap) {
            String msg = String.format(msgTpl, stockInfoMap.get("prod_name")
                    , stockInfoMap.get("last_px")
                    , stockInfoMap.get("high_px")
                    , stockInfoMap.get("low_px")
                    , stockInfoMap.get("px_change")
                    , stockInfoMap.get("market_value")
                    , stockInfoMap.get("amplitude") );
    ...
    public void sendOnlineUser(Map<String, User> onlineUser) {
        ...
        for(Map.Entry<String, User> entry : onlineUser.entrySet()){
            msg = msg.concat(entry.getValue().getUser()+" || ");
        }
